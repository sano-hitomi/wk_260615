...
if (length(ca_ids) == 0) {
  ...
  if (!is.na(name_col_diag)) {
    ...
    if (nrow(chol_rows) > 0) {
      print(chol_rows %>% dplyr::select(Alignment_ID, all_of(name_col_diag)) %>% head(20))
    } else {
...
intensity_wide <- bio %>%
  dplyr::select(label, Subject, Timepoint, period) %>%
...
write_csv(df_plot %>%
            dplyr::select(Subject, Timepoint, period,
