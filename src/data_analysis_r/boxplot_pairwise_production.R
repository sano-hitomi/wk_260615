...
# Pairwise timepoint comparisons (paired Wilcoxon) + box plots (production data)
#
# Comparisons:
...
#   data/production/processed/pairwise_test_results_{transform}_production.csv
#   output/figures/boxplot_sig_both_{transform}_production.pdf
#   output/figures/boxplot_sig_early_only_{transform}_production.pdf
#   output/figures/boxplot_sig_late_only_{transform}_production.pdf
#
# Usage (RStudio):
#   1. Run load_data.R first (samplesheet / feat_meta / feat_mat must exist)
#   2. source("src/r/boxplot_pairwise_production.R")
# =============================================================================
...
EXCLUDE_LOW_SCORE <- FALSE               # exclude "low score:" annotations
...
OUT_PROCESSED     <- "data/production/processed"
...
for (transform in TRANSFORMS) {
  ...
  # ---------------------------------------------------------------------------
  if (transform == "log2FC") {
    logfc_out <- file.path(OUT_PROCESSED, "log2FC_values_production.csv")
    logfc_long <- df_long %>%
  ...
  save_pdf_group(
    idx_both,
    file.path(OUT_DIR, sprintf("boxplot_sig_both_%s_production.pdf", transform)),
    "both", df_long, pval_df, classification, y_label
  )
  save_pdf_group(
    idx_early_only,
    file.path(OUT_DIR, sprintf("boxplot_sig_early_only_%s_production.pdf", transform)),
    "early_only", df_long, pval_df, classification, y_label
  )
  save_pdf_group(
    idx_late_only,
    file.path(OUT_DIR, sprintf("boxplot_sig_late_only_%s_production.pdf", transform)),
...
cat(sprintf("\n  Outputs:\n"))
cat(sprintf("    CSVs : %s/pairwise_test_results_*_production.csv\n", OUT_PROCESSED))
cat(sprintf("    PDFs : %s/boxplot_sig_*_production.pdf\n", OUT_DIR))
cat(sprintf("%s\n", strrep("=", 70)))
